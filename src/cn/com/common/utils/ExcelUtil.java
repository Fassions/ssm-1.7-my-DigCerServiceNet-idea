		...
		try {
			...
			if (rows > 0) {// 有数据时才处理
				...
				for (Field field : allFields) {
					// 将有注解的field存放到map中.
					if (field.isAnnotationPresent(ExcelVOAtteribute.class)) {
						ExcelVOAtteribute attr = field.getAnnotation(ExcelVOAtteribute.class);
				...
				for (int i = 1; i < rows; i++) {// 从第2行开始取数据,默认第一行是表头.
					...
					for (int j = 0; j <= cellNum; j++) {
						...
						if (cellType == HSSFCell.CELL_TYPE_NUMERIC) {
							...
							if ((""+cell.getNumericCellValue()).indexOf("E")!=-1
									|| (""+cell.getNumericCellValue()).indexOf("e")!=-1
									|| (""+cell.getNumericCellValue()).indexOf("+")!=-1) {
								BigDecimal bd = new BigDecimal(""+cell.getNumericCellValue());
								c = bd.toString();
							}else if(HSSFDateUtil.isCellDateFormatted(cell)){	//日期格式判断转换
								SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
								c = sdf.format(HSSFDateUtil.getJavaDate(cell.getNumericCellValue())).toString();
							}else{
...
//	public static String importByExcelForDate(Cell currentCell)
	...
	public ResultMessage exportUserProgress(List<T> ui,String sheetName,String basePath,String fileName,HttpServletRequest request) {
		ResultMessage resultMessage = new ResultMessage();
		FileOutputStream out = null;
		//String basePath = CommonProperty.applicationProperties.getProperty("httpfilerootOrderTemp");
//		String basePath = request.getSession().getServletContext().getRealPath("/");
		fileName = fileName==null?DateUtils.stapToFileNameDate(System.currentTimeMillis()+""):fileName;
		String filename = basePath + fileName + ".xls";
		try{
			out = new FileOutputStream(filename);
		}catch (FileNotFoundException e){
			e.printStackTrace();
		}
		boolean exportExcel = exportExcel(ui, sheetName, out);//导出
		if (exportExcel) {
			resultMessage.setResult(ResultMessage.Result.OUTSUCCESS);
			resultMessage.setErrorMsg(filename);
		}
